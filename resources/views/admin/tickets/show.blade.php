...
@section('content')
<div class="admin-hero"><div><h2>{{ $ticket->subject }}</h2><div class="muted">{{ $ticket->number }} · {{ $ticket->email }}</div><span class="pill">SUPPORT</span></div><span class="pill">{{ $ticket->status }}</span></div>
