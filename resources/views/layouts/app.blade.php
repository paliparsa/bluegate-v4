...
<aside class="card sidebar">
 ...
 <nav class="side">
  ...
  <div class="side-section">ارتباط و مزایا</div>
  ...
  @if(in_array(auth()->user()->role,['admin','super_admin'],true))
   ...
   <a href="{{ route('admin.dashboard') }}">داشبورد مدیریت</a><a href="{{ route('admin.nodes') }}">نودها</a>
   <a href="{{ route('admin.wallets') }}">کیف پول‌ها</a><a href="{{ route('admin.payments') }}">پرداخت‌ها</a>
   <a href="{{ route('admin.products') }}">محصولات</a><a href="{{ route('admin.coupons') }}">تخفیف‌ها</a>
   <a href="{{ route('admin.tickets') }}">تیکت‌ها</a><a href="{{ route('admin.resellers') }}">Resellers</a><a href="{{ route('admin.analytics') }}">Analytics</a>
  @endif
 </nav>
</aside>
<section class="content">
 @if(session('success'))<div class="alert success" style="margin-top:20px">{{ session('success') }}</div>@endif
 @if(session('error'))<div class="alert" style="margin-top:20px">{{ session('error') }}</div>@endif
 @yield('content')
</section>
</main>
